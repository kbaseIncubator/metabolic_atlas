package Bio::KBase::MetaboliteAtlas::Service;
...
sub call_method {
    ...
    my $ctx = Bio::KBase::MetaboliteAtlas::ServiceContext->new(client_ip => $self->_plack_req->address);
    
    my $args = $data->{arguments};

{
    # Service MetaboliteAtlas requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for MetaboliteAtlas but no authentication header was passed");
	}
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service MetaboliteAtlas.");
    }
...
package Bio::KBase::MetaboliteAtlas::ServiceContext;

use strict;

=head1 NAME

Bio::KBase::MetaboliteAtlas::ServiceContext
